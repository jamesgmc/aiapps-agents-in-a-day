...
3.  Open `setup.sh`, let's update `<DEV_Name>` setting with your own database name. Replace `add_value` with `aiaaa_xxxxx_xx` (for example: aiaaa_daniel_66).
...
4. **Use Copilot Agent Mode to Update the Script Automatically**
   ...
   - Paste the values into the Copilot chat and ask Copilot to replace the placeholder values in your setup script.
        Prompt: Update the setup.sh file by replacing the values for ["<MONGODB_CONNECTION_STRING>"], ["<AZURE_OPENAI_API_INSTANCE_NAME>"], ["<AZURE_OPENAI_API_KEY>"], and ["<APPLICATIONINSIGHTS_CONNECTION_STRING>"] with the provided values above.
   - If successful, Copilot will update the script for you—no manual editing needed!

   Go to terminal windows, save the `setup.sh` file and run below bash command to replace all variables in the repo. If you need to run `setup.sh` again to fix typos, you need to revert previous changes in `Source Control` tab or ask us for help.
...
5. **Backup Script: Auto-Pull Keys from URL**
    ...
    - Search for `<DEV_Name>` and replace with `aiaaa_xxxxx_xx` (for example: aiaaa_daniel_66). 
...
7.  Install node packages for test program.

    ```bash
    npm install
    ```

8.  Test Connection to CosMos Db and OpenAI service. Run the following command in the terminal window to execute the script:

    ```bash
    node test.js
    ```

9.  If you see below message in the console, you are ready to go! otherwise, please `Raise your hand and ask for help`. Note, you do not need to commit any changes of the `.env` files to Git repo.
