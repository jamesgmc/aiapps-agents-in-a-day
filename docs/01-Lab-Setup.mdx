...
https://github.com/GitHub-Insight-ANZ-Lab/aiapps-agents

The repository contains the following folders:

- Lab exercises: `~/labs`
- Node.js Backend API: `~/apps-chat/backend`
- Frontend Web App: `~/apps-chat/chatbot`
...
2. **Open in Codespaces**: Navigate to the [aiapps-agents](https://github.com/GitHub-Insight-ANZ-Lab/aiapps-agents) repository. Click on the "Code" button and select "Open with Codespaces". If you don't see this option, ensure you have GitHub Codespaces enabled for your account.
...
2. Visit our [aiapps-agents](https://github.com/GitHub-Insight-ANZ-Lab/aiapps-agents) repository in GitHub. This repository contains the source code and guides for completing the labs.

    ```bash
    git clone https://github.com/GitHub-Insight-ANZ-Lab/aiapps-agents.git
    ```
...
3.  Open `setup.sh`, let's update `<MONGODB_Name>` setting with your own database name. Replace `add_value` with `aiaaa_xxxxx_xx` (for example: aiaaa_daniel_66).
...
4.  Update `<MONGODB_CONNECTION_STRING>` with `mongodb+srv://aiaaaadmin:aiaaapassword123@arg-syd-aiaaa-mongo.global.mongocluster.cosmos.azure.com/?tls=true&authMechanism=SCRAM-SHA-256&retrywrites=false&maxIdleTimeMS=120000`

5.  Update `<AZURE_OPENAI_API_INSTANCE_NAME>` with `arg-syd-aiaaa-openai`

6.  Update `<AZURE_OPENAI_API_KEY>` with `a30df7e6e63f424884fde2f86b2624b5`

7.  Update `<GRAPH_RAG_API>` with `https://arg-syd-aiaaa-ca.ashycoast-2c2322dd.eastus.azurecontainerapps.io`
...
8.  Go to terminal windows, save the `setup.sh` file and run below bash command to replace all variables in the repo. If you need to run `setup.sh` again to fix typos, you need to revert previous changes in `Source Control` tab or ask us for help.
    ...
    - Search for `<MONGODB_Name>` and replace with `aiaaa_xxxxx_xx` (for example: aiaaa_daniel_66). 
...
| AZURE_OPENAI_API_ENDPOINT                   | https://arg-syd-aiaaa-openai.openai.azure.com/ |
...
| ------------- | ------------- | ------------- |
| GPT-4o | https://arg-syd-aiaaa-openai.openai.azure.com/openai/deployments/gpt4o/chat/completions?api-version=2024-08-01-preview | a30df7e6e63f424884fde2f86b2624b5 (deploymentName: gpt4o) |
| Dall-e 3 | https://arg-syd-aiaaa-openai.openai.azure.com/openai/deployments/dalle3/images/generations?api-version=2024-02-01 | a30df7e6e63f424884fde2f86b2624b5 (deploymentName: dalle3) |
| Computer Vision | https://arg-syd-aiaaa-cv.cognitiveservices.azure.com/ | 786639364fd04642b86ae805c72c43f6 |
